...
internal sealed class GetPostsByUserIdQueryHandler(
    IApplicationDbContext context,
    IMapper mapper,
    ICacheService cacheService,
    IUserContext userContext)
    : IQueryHandler<GetPostsByUserIdQuery, PaginatedByOffsetListDto<PostDto>>
{
    public async Task<Result<PaginatedByOffsetListDto<PostDto>>> Handle(GetPostsByUserIdQuery query, CancellationToken cancellationToken)
    {
        ...
        string cacheKey = $"posts:userId:{query.UserId}:{query.Page}:{query.PageSize}";
